...
void mqtt_server_connect(lv_event_t * e)
{
	...
	if (mqtt_server == NULL || mqtt_port == NULL || mqtt_user == NULL || mqtt_passwd == NULL) {
	...
	// Connect按钮变绿色并禁用
    lv_obj_set_style_bg_color(ui_MqttConnect, lv_color_hex(0x4CAF50), LV_PART_MAIN);
    lv_obj_add_state(ui_MqttConnect, LV_STATE_DISABLED);

    // Disconnect按钮变红并启用
    lv_obj_set_style_bg_color(ui_MqttDisconnect, lv_color_hex(0xF44336), LV_PART_MAIN);
...
{
	// Disconnect按钮变蓝并禁用
    lv_obj_set_style_bg_color(ui_MqttDisconnect, lv_color_hex(0x2196F3), LV_PART_MAIN);
    lv_obj_add_state(ui_MqttDisconnect, LV_STATE_DISABLED);

    // Connect按钮变蓝并启用
    lv_obj_set_style_bg_color(ui_MqttConnect, lv_color_hex(0x2196F3), LV_PART_MAIN);
